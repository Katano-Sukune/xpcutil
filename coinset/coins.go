...
func (cs *CoinSet) TotalValue() (value xpcutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue xpcutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue xpcutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount xpcutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue xpcutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue xpcutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue xpcutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue xpcutil.Amount, coins []Coin) (Coins, error) {
...
// xpcutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() xpcutil.Amount {
	return xpcutil.Amount(c.txOut().Value)
}
